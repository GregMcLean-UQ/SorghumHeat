...
{
	// calculate a daily temperature contribution to stress on grain number (tempFactor)
	// // If DCaPST then we look at leaf temperature (boot) effect on grain number then the air temp effect
	// 
	double heatSeverity = 0;
	double dltTT = plant->phenology->getDltTT();
	if (plant->today.leafTemp > 0)      // if using DCaPST
	{
		double ttToFlag = plant->phenology->sumTTtarget(sowing, flag);
		double eTT = plant->phenology->sumTTtotal(sowing, flowering);       // tt to now
		if (eTT > ttToFlag + leafTempWindow[0] && eTT < ttToFlag + leafTempWindow[1])  // In the window
		{
			heatSeverity = bootTempTable.value(plant->today.leafTemp);
			double dayFract = dltTT / (-leafTempWindow[0] + leafTempWindow[1]);
			//return dayFract * heatSeverity;
			return heatSeverity;
		}
	}
	// 
	// if we are within the air temp grain stress window (grainTempWindow)calculate stress factor
	// from grainTempTable and this day's contribution to the total stress

	 // calc severity
	heatSeverity = grainTempTable.value(plant->today.maxT);
...
// If grain number is reduced due to heat keep demand calculation on finalGrainNo
double Grain::calcDMGrainSourceSink(void)
{
	double unstressedGrainNumber = grainNo / tempFactor;
	double totDMCaryopsis = divide(plant->biomass->getDltDM(), unstressedGrainNumber);
